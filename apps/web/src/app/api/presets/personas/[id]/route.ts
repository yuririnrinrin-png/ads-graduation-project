...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const existing = await prisma.presetPersona.findUnique({ where: { id } });
  if (!existing) {
  ...
  if (file instanceof File && file.size > 0) {
    try {
      data.imageKey = await savePresetImage("personas", id, file);
    } catch (e) {
      return NextResponse.json(
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  const remaining = await prisma.presetPersona.count();
  if (remaining <= 1) {
    return NextResponse.json(
      { error: "人物は最低1人残してください" },
      { status: 409 }
    );
  }
  const used = await prisma.job.count({ where: { personaId: id } });
  if (used > 0) {
    return NextResponse.json(
      { error: "この人物を使っているジョブがあるので消せません" },
      { status: 409 }
    );
  }

  await prisma.presetPersona.delete({ where: { id } });
