...
| 4 Scene ×7 | 着用 4＋全身 2＋引き 1。同一 persona。ジュエリーは描かせない。**胴体は正面±15°、顔は斜め・横向き可**。表情は7枠すべて別。デートは対面の男性（ぼかし）。髪型はアップ＋耳かけダウン（カフェ／引き）。服は首元開きとタートルを混在。リング／ブレス着用は手元主役。**人物のみのシーン画像を中間成果物として保存** |
...
| 4 | インセット・枠再生成・失敗段階リトライ | **着手中** — レビュー操作とプリセット追加 CRUD は実装済み。進捗磨きは未 |
