...
- 着用系は大きさ・位置スライダー（人物は再生成しない）。角度の自動補正はしない
- インセット枠: ディテール選択の組み直し
- ZIP ダウンロード
...
| 5 Composite | 実物切り抜きをカテゴリ別アンカーに配置。transform 反映可。中間の人物シーンを再利用 |
| 6 Inset | 引き＋右下ディテール |
...
方向性は「参照画像で顔を固定する系」（IP-Adapter FaceID / PuLID / InstantID 等）。fal.ai で使えるかを小さくスパイクしてからモデルを確定する（[REQUIREMENTS.md](REQUIREMENTS.md) 未決 2）。
...
| 1 | monorepo 骨組み・認証・ジョブ CRUD・ダミー 10 枚 ZIP | **着手中** — `apps/web` / `apps/worker` / `packages/shared` |
