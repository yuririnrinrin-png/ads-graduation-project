...
待ち画面はいまの段階・経過時間・「画面を閉じても続く」を出す。途中画像の先出しはしない。
...
| 4 Scene ×7 | 着用 4＋全身 2＋引き 1。同一 persona。ジュエリーは描かせない。**胴体は正面±15°、顔は斜め・横向き可**。表情は7枠すべて別。デートは対面の男性（顔が見える、二人の雰囲気、主役は女性）。髪型はアップ＋耳かけダウン（カフェ／引き）。服は首元開きとタートルを混在。リング／ブレス着用は手元主役。**人物のみのシーン画像を中間成果物として保存** |
...
3. 着用・全身・引きの7枚はすべて同じ参照で PuLID 生成（プロンプトでジュエリー禁止。胴体は正面寄り、顔の向きは枠ごとに変える）

実装: `apps/worker/worker/scene_gen.py`。コスト上限は [REQUIREMENTS.md](REQUIREMENTS.md) §9（1ジョブ200円）。
...
| 項目 | 方針 |
|---|---|
| 画像生成 API コスト | 1ジョブ **200円**（初回＋同じジョブの枠再生成）。超えたらそのジョブを停止。円換算は固定 150円/USD。fal 単価は `flux/dev` $0.025/MP・`flux-pulid` $0.0333/MP（1024四方は切り上げ 2MP → 約8円/約10円）。`USD_JPY_RATE` と `FAL_USD_PER_MP_*` で更新 |
| API キー | ワーカー側の環境変数のみ。ブラウザや Next.js 公開バンドルに載せない |
| アップロード | サーバ側で形式・サイズを検証 |
| 保管 | 生成ファイルは作成から **14 日**で削除。ジョブ行は `expired` のまま残る。ZIP は都度打包 |
| 死活確認 | `GET /api/health`（ログイン不要。DB が生きていれば 200） |
...
| 3 | 人物シーン生成＋実物合成＋ transform | **完了** — 合成・ドラッグ・ピアス両耳・fal.ai・影＋チルト・顔矩形配置・シーン多様化済み（キーなしは仮。persona 本番写真は `imageKey` があれば使用） |
| 4 | インセット・枠再生成・失敗段階リトライ | **完了** — レビュー操作・プリセット CRUD・進捗磨き（段階名・目安・離脱可） |
| 5 | 画質・リトライ・14 日削除・デプロイ固め | **進行中** — 14日削除・1ジョブ200円・学校用 Vercel 公開まで。ワーカーは提出までこのPC |

## Phase 0 モック（旧・残置）

- ローカル: [mockups/product-ui.html](mockups/product-ui.html)
- 公開: https://ti-amo-jewelry-studio.surge.sh
- 内容: ログイン／新規ジョブ／進捗（成功・失敗）／レビュー。実 API なし

## 現行 UI モック（Phase 5 時点）

- ローカル: [mockups-v2/index.html](mockups-v2/index.html)
- 公開: https://ti-amo-jewelry-studio-v2.surge.sh
- 内容: 本実装と同じ骨格。一覧・プリセット・14日削除・1ジョブ200円・位置のドラッグ／回転／ピアス片方消し・停止時リトライを含む。実 API なし。旧 URL は上書きしない
...
| 2026-08-14 | 胴体は正面のまま顔は斜め・横向き可。伏し目・柔らかい目線。ピアスは髪オーバーレイなし |
| 2026-08-16 | シーン仕様: 表情7種・デートの対面男性・タートル混在・リング／ブレス手元主役。`.cursor/hooks` でプロンプト検査 |
| 2026-08-17 | Phase 4: プリセット最小 CRUD（人物・背景・トーンの追加／改名／削除。人物・背景は写真アップロード可） |
| 2026-08-21 | Phase 4 完了: 進捗画面を要件§7の5段階に揃え、目安・経過・離脱可を表示 |
| 2026-08-22 | Phase 5: 1ジョブ200円で停止。学校用 Vercel 公開（画面+Neon）。ワーカーは提出までこのPC |
| 2026-08-22 | 現行 UI モックを別 URL で公開（旧 Phase 0 は残置） |
